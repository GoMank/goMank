...
import { GET_NEAREST_MAMANG, GET_UPDATED_LOCATION_MAMANG } from "../../config/queries";
import axios from "axios";
export default function Maps() {
  const [address, setAddress] = useState([]);
  const [location, setLocation] = useState(null);
  const [errorMsg, setErrorMsg] = useState(null);
  const [mamang, setMamang] = useState({})
  const [nearestMamang, {
    data: mamangLoc,
    loading: mamangLoading,
    error: mamangtError },
  ] = useMutation(GET_NEAREST_MAMANG);
  let t
  useEffect(() => {
    t = setInterval(() => {
      (async () => {
        const {data} = await axios.get('https://slippery-newt-22.loca.lt/mamangs/62559dcfc9054d53a273fb14')
        if(data){
          setMamang(data)
        }
      })();

...
const {connection, getData, client} = require('../config/mongoDb')
...
describe(`GET /clients/:id--success`, () => {
    it('should return 200 ', async () => {
        ...
        expect(res.status).toBe(200)
        expect(res.body).toHaveProperty('_id', '625584887e8eeed032147b55')
...
describe(`GET /clients/login --failed invalid password`, () => {
    it('should return 400 ', async () => {
        // await connectMongoDb()
        const body = { email: "testing99@mail.com", password: "1234" }
        const res = await request(app).post('/clients/login').send(body)
